...
## Result (2026-05-13) — shipped (commit `76cd8f6`); marginal improvement; two follow-up bugs

Implemented, Test_PositionManager 35/35 green (+4 `SLDistanceFloor` asserts), EA compiles 0/0. Re-baseline run (7-symbol default, 12-mo, `LipsBreak_Confirm_Bars=2`, `Verbose_Logging=false`): final balance **$95,603 = −4.40%**, 279 trades — better than the 9-symbol −5.24% but **still a loss**. Per-symbol trade P/L: NZDUSD +$1.9k, USDJPY +$1.8k vs GBPUSD −$2.4k, USDCHF −$2.0k, XAUUSD −$1.8k, EURUSD −$0.9k (net ≈ −$3.4k trade-P/L; the rest ~$1.0k swap). The account improved mostly via *less swap* (fewer trades/holds), not better selection — and `GBPUSD` flipped from +$1.4k (9-symbol run) to −$2.4k here purely from the symbol-set change (the global single-trade rule couples the symbols → per-symbol P/L isn't cleanly attributable; re-pick the symbol set holistically after Stage 2).

Three findings → **Stage 1.1 patch** (bug-fixes, no new design):
1. **NAS100/USTEC still never trades** — `Spread_NAS100=200` is still below the broker's USTEC spread at the (mostly off-US-hours) bars where signals fire; every NAS100 signal is `spread=NO`. → drop `NAS100` from the `Trade_Symbols` default — it's effectively untradeable here. (So the "7-symbol" run was really 6-symbol: EURUSD, GBPUSD, USDJPY, USDCHF, NZDUSD, XAUUSD.)
2. **~22 orders still PLACE_FAILED `Invalid stops`** — traced to a pre-existing edge-case where `CSignalEngine` produces an `sl_price` on the *wrong side* of the entry (e.g. A_SELL with SL *below* entry). Rare, self-protecting, ugly. → add a "SL on the correct side of entry" guard in `BuildPlan`: BUY → reject if `out.sl >= out.entry`; SELL → reject if `out.sl <= out.entry`. Add a `Test_PositionManager` assert (BuildPlan is live-only, so test the guard via a tiny pure helper, or just integration-verify).
3. **`Min_SL_ATR_Mult=1.0` over-rejects** — it threw out ~110 signals in the run, likely including legit Type-A entries whose structural SL is ~0.6×ATR from entry. → dial `Min_SL_ATR_Mult` `1.0 → 0.3` (still well above any broker stops level / lot-blow-up risk, but doesn't kill legit setups).
