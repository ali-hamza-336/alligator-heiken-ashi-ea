...
| `Trade_Symbols` | CSV of canonical names — default `"EURUSD,GBPUSD,USDJPY,USDCHF,AUDUSD,USDCAD,NZDUSD,XAUUSD,NAS100"`. Resolved to broker symbols at `OnInit` via `SymbolSelect()` probing; the EA refuses to start (with a clear log) if any name is unresolvable. Known broker alias: **`NAS100` → `USTEC` on IC Markets**; all other 8 match exactly. For a single-symbol backtest set this to just `"EURUSD"`. |
...
| `Spread_EURUSD` … `Spread_NAS100` | Per-symbol max spread (entry skipped if `current_spread > limit`). The §9 defaults are tight on some brokers — e.g. **IC Markets quotes `USTEC` at ~90 points** vs the default `Spread_NAS100 = 2`, and gold spreads vary. Watch the per-bar `filters: … spread=OK/NO …` log lines after attaching, and raise the relevant `Spread_*` input per broker. (The Strategy Tester models its own spreads, so these only matter live.) |
